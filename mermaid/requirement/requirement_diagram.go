...
func NewDiagram(w io.Writer, opts ...Option) *Diagram {
	...
	if title != noTitle {
		...
		lines = append(lines, "---")
		lines = append(lines, internal.FrontMatterTitle(internal.FoldFrontMatterTitleCR(title)))
		lines = append(lines, "---")
	}
...
// quote returns value as the double quoted string a requirement diagram takes.
//
...
// visible "\t" spelling without the stray "&". A bare "<" is written as
// "#60;" by EscapeBareAngle, because the sanitizer read "a<b" in a
// requirement name as an opening tag and drew "a". A line break is written as
...
func quote(value string) string {
	escaped := internal.EscapeEntityOpeners(value)
	escaped = strings.ReplaceAll(escaped, `\`, "#92;")
	escaped = strings.ReplaceAll(escaped, "\r\n", "<br/>")
	escaped = strings.ReplaceAll(escaped, "\r", "<br/>")
	escaped = strings.ReplaceAll(escaped, "\n", "<br/>")
	escaped = internal.EscapeBareAngle(escaped)
